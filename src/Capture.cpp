...
QString Capture::byteToHex(unsigned char *str, int size)
{
    QString res = "";
...
void Capture::run()
{
    ...
    while (true) {
        ...
        if(res == 0)
            continue;
        localTimeVersionSec = header->ts.tv_sec;
        localtime_s(&localTime, &localTimeVersionSec);
...
int Capture::ethernetPackageHandle(const u_char *pkt_content,QString& info)
{
    ...
    ethernet_type = ntohs(ethernet->ethernetType);
...
*/
    ...
    ipPackage = (htons(ip->totalLength) - (ip->versionHeaderLength & 0x0F) * 4);
    return protocol;
}
...
{
    ICMPHEADER*icmp;
    icmp = (ICMPHEADER*)(pkt_content + 20 + 14);
...
int Capture::tcpPackageHandle(const u_char *pkt_content, QString &info, int ipPackage)
{
    ...
    u_short des = ntohs(tcp->desPort);
    QString proSend = "";
    QString proRecv = "";
    int type = 3;
    int delta = (tcp->headerLength >> 4) * 4;
...
{
    UDPHEADER *udp;
    udp = (UDPHEADER*)(pkt_content + 14 + 20);
    u_short desPort = ntohs(udp->desPort);
    u_short srcPort = ntohs(udp->srcPort);
    if(desPort == 53){ // dns query
        ...
        res += " len=" + QString::number(ntohs(udp->dataLength));
        info = res;
        return 4;
    }
}
...
QString Capture::arpPackageHandle(const u_char *pkt_content)
{
    ...
    u_char* srcEthTemp = arp->srcEthAddr;

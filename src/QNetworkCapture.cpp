    ...
    , ui(new Ui::QNetworkCapture)
{
    ...
    connect(ui->actionStartCapture, &QAction::triggered, this, [=]{
        ...
        if(index) {
            if(ui->tableWidget->rowCount()) {
                ...
                msgBox.setText("Before restarting do you want to save result?");
                msgBox.setStandardButtons(QMessageBox::Save | QMessageBox::Cancel);
                msgBox.setDefaultButton(QMessageBox::Save);
                int type = msgBox.exec();
                if(type == QMessageBox::Save) {
                    // save the file
                } else if (type == QMessageBox::Cancel) {
                    index = !index;
                    isStart = false;
                    return;
                }
            ...
            int res = capture();
            if(pointer && res != 1) {
                captureThread->resetFlag();
                captureThread->setPointer(pointer);
                ui->comboBox->setEnabled(false);
                captureThread->start();
                ui->actionStartCapture->setIcon(QIcon(":/Image/stop.png"));
                countNumber = 0;
                isStart = true;
            } else { // fail to start
                index = !index;
            ...
            captureThread->setFlag();
            ui->comboBox->setEnabled(true);
            captureThread->quit();
            captureThread->wait();
            pcap_close(pointer);
            ui->actionStartCapture->setIcon(QIcon(":/Image/start.png"));
            isStart = false;
        }
    });
    ...
    connect(ui->actionClearAll, &QAction::triggered, this, [=]{
        if(!isStart) {
            QMessageBox msgBox(this);
            msgBox.setWindowTitle("information");
            msgBox.setText("Do you want to clear all?");
    ...
    connect(captureThread, &Capture::send, this, &QNetworkCapture::handleMessage);
...
int QNetworkCapture::capture()
{
    if(device) {
        ...
        ui->statusbar->showMessage("please chose the Network Card!");
        return -1;
    }

    if(!pointer) {
        ui->statusbar->showMessage(QString(errbuf));
        ...
        ui->statusbar->showMessage(device->name);
    }
    return 0;
}
...
*/
void QNetworkCapture::handleMessage(DataPackage data)
{
...
void QNetworkCapture::on_tableWidget_cellClicked(int row, int column)
{
...
*/
void QNetworkCapture::on_lineEdit_returnPressed()
{
    ...
    ui->statusbar->showMessage("Have show (" + QString::number(count) + ") " +QString::number(res,10,2) + "%");
}
...
*/
void QNetworkCapture::on_lineEdit_textChanged(const QString &arg1)
{
    QString text = arg1;
    text = text.toLower();
...
*/
void QNetworkCapture::on_tableWidget_currentCellChanged(int currentRow, int currentColumn, int previousRow, int previousColumn)
{
